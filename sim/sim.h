...
public:
  ...
  //BW stats
  int bw_epoch=10;
  int curr_bytes_accessed=0;
  vector<float> bw_per_epoch;
  ...
  // Graphfire
  unordered_map<int, tuple<int, int, int>>* RRVT; // = new unordered_map<int, tuple<int, int, int>>(); //track reuse of PCs
  bool learningRRVT=false; //is learning
  int learningTime=0; //learning time
  int LEARNING_THRS=0; //threshold for learning time
  bool learningFreq=true; //learning freq
  bool foundPIA=false; //found PIA
  ...
  void evictAllCaches(DynamicNode* d);
