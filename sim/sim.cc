...
Simulator::Simulator(string home) {
  ...
  cache = new Cache(cfg);
  cache->sim = this;
  cache->fc->setGraphfire(RRVT, &learningRRVT);
  //cache->isLLC=true;
  cache->memInterface = memInterface;
 
  // LLAMA Cache
  llama_cache = new Cache(cfg.cache_latency, 0, cfg.bank_size, cfg.llama_cache_linesize, cfg.llama_cache_linesize, cfg.llama_cache_load_ports, cfg.llama_cache_store_ports, cfg.llama_ideal_cache, cfg.llama_prefetch_distance, cfg.llama_num_prefetched_lines, cfg.llama_cache_size, cfg.llama_cache_assoc, cfg.llama_eviction_policy, 0, cfg.use_l2, 2, 0, cfg.cache_by_temperature, cfg.node_degree_threshold, 0, cfg.is_llc, cfg.is_l1, 0, 0, 3, 16, 63, 0);
...
bool Simulator::isLocked(DynamicNode* d) {
  ...
  /*int nodeId = d->n->id;
  if (d->core->graphfire > 0 && !learningRRVT && get<0>((*(RRVT))[nodeId]) < RRV_THRS) {
    cacheline=d->addr;
  }*/
...
evictAllCaches(d); //upon assignment of lock, must evict all cachelines  
...
bool Simulator::lockCacheline(DynamicNode* d) {
  ...
  if(!isLocked(d)) {
    evictAllCaches(d); //pass in address, not cacheline
  }
...
void Simulator::evictAllCaches(DynamicNode* d) {
  uint64_t addr = d->addr;
  for(auto id_tile: tiles) {
    if(Core* core=dynamic_cast<Core*>(id_tile.second)) {
      if (core->id == d->core->id) {
        continue;
      }
      core->cache->evict(addr, false, core->id); 
      if (core->cache->useL2) {
        core->cache->parent_cache->evict(addr, false, core->id);
      }     
    }
  }
    ...
    if(tiles[0]->cycles % stat.printInterval == 0 && tiles[0]->cycles !=0) {
      ...
      if(evictStatsVec.size() > 0) { 
        ...
        if (!evictStatsIn) {
          ...
          evictStats << "Cacheline\tNew Node ID\tOffset\tEviction Cycle\tNode ID\tGraph Node ID\tGraph Node Deg\tUnused Space\tCache Level" << endl;
        }

        evictOutstring = "";
        for(auto it = evictStatsVec.begin(); it != evictStatsVec.end(); it++) {
          auto entry = *it;
          evictOutstring += to_string(entry.cacheline) + "\t" + to_string(entry.newNodeId) + "\t" + to_string(entry.offset) + "\t" + to_string(entry.cycle) + "\t\t" + to_string(entry.nodeId) + "\t" + to_string(entry.graphNodeId) + "\t\t" + to_string(entry.graphNodeDeg) + "\t\t" + to_string(entry.unusedSpace) + "\t\t" + to_string(entry.cacheLevel) + "\n";
        }
...
*/
  ...
  if (memStatsIn) {
    ...
    if(accesses_per_epoch.size()==0) { // CHECK THIS
    ...
    /*ofstream bwStats;
    bwStats.open(outputDir + "bwStats");
    for(auto it = bw_per_epoch.begin(); it != bw_per_epoch.end(); it++) { 
      auto access = *it;
      bwStats << access << endl;
    }
    bwStats << endl;

    string median_bw_prefix="";
    median_bw_prefix+= "Median Bandwidth Per " + to_string(bw_epoch) + "-cycle Epoch: ";
    string max_bw_prefix="";
    max_bw_prefix+= "Max Bandwidth Per " + to_string(bw_epoch) + "-cycle Epoch: ";
    string mean_bw_prefix="";
    mean_bw_prefix+= "Mean Bandwidth Per " + to_string(bw_epoch) + "-cycle Epoch: ";
    sort(bw_per_epoch.begin(), bw_per_epoch.end());
  
    if(bw_per_epoch.size()==0) { // CHECK THIS
      bwStats << mean_bw_prefix << 0 << endl;
      bwStats << median_bw_prefix << 0 << endl;
      bwStats << max_bw_prefix << 0 << endl;
    } else {
      bwStats << mean_bw_prefix << ((float) accumulate(bw_per_epoch.begin(), bw_per_epoch.end(),0))/bw_per_epoch.size() << endl;
      bwStats << median_bw_prefix << bw_per_epoch[bw_per_epoch.size()/2] << endl;
      bwStats << max_bw_prefix << bw_per_epoch[bw_per_epoch.size()-1] << endl;
    }
